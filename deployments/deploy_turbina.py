...
endpoint_name = "turbina-endpoint"
...
try:
    ...
    endpoint = ManagedOnlineEndpoint(
        name=endpoint_name,
        description="Endpoint para modelo turbina",
        auth_mode="key"
    )
...
deployment = ManagedOnlineDeployment(
    ...
    model=f"azureml:turbina_model_completo:1",  # Asegúrate del número de versión
    environment=environment,
    instance_type="Standard_DS2_v2",
    instance_count=1,
    code_configuration=CodeConfiguration(
        code="./deployments/turbina",
